...
> **rebase/マージ時の重複に注意**: 並行して作業している別ブランチが同じ意図のマイグレーションを
> 別のファイル名(別タイムスタンプ)で追加していると、タイムスタンプが異なるためCIの重複検知
> (`.github/workflows/ci.yml`の`migration-lint`)をすり抜け、同一内容のマイグレーションが2つ
> 履歴に残ってしまうことがある(#209)。rebase/マージ後は`supabase/migrations/`に意図せず
> 同名サフィックスのファイルが増えていないか確認すること。CIはPRで新規追加されたマイグレーション
> についてのみ、既存ファイルとの同名サフィックス重複を検知する。
